...
switch ($_GET['type']) {
   case 'all':
      ...
      break;
   case 'nameonly':
      $sqlString = "SELECT id, user_firstname, user_lastname, user_middlename, user_dob,
      user_sex, user_address, user_city, user_province  FROM `users` WHERE id = '{$_GET['userId']}'";
      $rs = mysqli_query($con, $sqlString);
